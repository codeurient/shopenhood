...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">Update "{{ $category->name }}"</p>
        </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" id="categoryForm">
                        ...
                        <div>
                            ...
                            @if($category->image)
                                <div class="mb-2 flex items-center gap-3">
                                    <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-16 h-16 object-cover rounded border">
            ...
            <div class="px-8 py-6 bg-gray-50 border-t border-gray-200 flex justify-between">
                <button type="button" onclick="confirmDelete()"
                        class="inline-flex items-center px-6 py-2.5 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 transition">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        .then(data => {
            if (data.success && data.categories.length > 0) {
                ...
                data.categories.forEach(category => {
                    // Skip the current category to prevent selecting self as parent
    ...
    function restoreHierarchy() {
        ...
        // Fetch the hierarchy for the current parent
    ...
    function selectHierarchyLevel(hierarchy, index) {
        ...
        // If there are more levels, create next select and load children
...
function confirmDelete() {
    if (confirm('Are you sure you want to delete this category? This will also delete all child categories and remove variant assignments.')) {

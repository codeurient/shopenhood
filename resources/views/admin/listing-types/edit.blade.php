...
<div>
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-[#1A1A1A]">Edit Listing Type</h2>
            <p class="text-[#37474F] text-sm mt-1">{{ $listingType->name }}</p>
        </div>
    ...
    @if($listingType->listings_count > 0)
        <div class="mb-5 p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded flex items-center gap-3">
            <i class="fa-solid fa-triangle-exclamation text-yellow-500 flex-shrink-0"></i>
            <p class="text-[13px] text-yellow-700">
                This listing type has <strong>{{ $listingType->listings_count }}</strong> listing(s) associated with it. Be careful when making changes.
            </p>
        </div>
    @endif
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              class="block p-3 w-full text-[13px] text-[#1A1A1A] rounded border focus:ring-0 focus:border-[#D4AF37] @error('description') border-red-500 @else border-[#E0E0E0] @enderror">{{ old('description', $listingType->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-[12px] text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div id="icon-preview"
                            ...
                            {!! old('icon', $listingType->icon) ?: '<i class="fa-solid fa-tag"></i>' !!}
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order"
                           value="{{ old('sort_order', $listingType->sort_order) }}" min="0"
                    ...
                    <div class="flex items-start gap-4">
                        <label for="requires_price" class="inline-flex items-center gap-3 cursor-pointer mt-0.5">
                            <input type="checkbox" name="requires_price" id="requires_price" value="1"
                                   {{ old('requires_price', $listingType->requires_price) ? 'checked' : '' }}
                                   class="sr-only peer">
                    ...
                    <div class="flex items-start gap-4">
                        <label for="is_active" class="inline-flex items-center gap-3 cursor-pointer mt-0.5">
                            <input type="checkbox" name="is_active" id="is_active" value="1"
                                   {{ old('is_active', $listingType->is_active) ? 'checked' : '' }}
                                   class="sr-only peer">
                            <div class="relative w-11 h-6 bg-[#E0E0E0] peer-focus:outline-none rounded-full peer
                                        peer-checked:after:translate-x-full peer-checked:after:border-white
                                        after:content-[''] after:absolute after:top-[2px] after:start-[2px]
